...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    // Copy variables within scoped mutex
    {
        ...
        else if (mState == Tracking::OK)
        {
            ...
            currentFrame = mCurrentFrame; // Copy the whole frame to access custom vectors
    ...
    {
        // ... (Initialization drawing code remains unchanged) ...
    ...
    else if (state == Tracking::OK) // TRACKING
    {
                ...
                if (useReliability)
                {
                    ...
                    if (rel > 0.7f)
                    {
                        ...
                        if (vbMap[i])
                            ...
                            drawColor = cv::Scalar(0, 255, 255); // Cyan (VO but reliable)
                    }
                    else if (rel > 0.3f)
                    {
        ...
        if (useReliability)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                if ((vbVO[i] || vbMap[i]) && currentFrame.mvStaticReliability[i] < 0.3f)
                    dynamicCount++;
            }
            char info[100];
